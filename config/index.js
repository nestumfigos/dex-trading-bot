...
const config = {
  ...
  strategies: {
    ...
    swing: {
      ...
      rsiBuyThreshold: parseFloatEnv(['SWING_RSI_BUY_THRESHOLD', 'STRATEGY_SWING_RSI_BUY_MIN'], 35),
      rsiBuyMaxThreshold: parseFloatEnv(['SWING_RSI_BUY_MAX_THRESHOLD', 'STRATEGY_SWING_RSI_BUY_MAX'], 90),
      volumeSpikeMultiplier: parseFloatEnv(['SWING_VOLUME_SPIKE_MULTIPLIER', 'STRATEGY_SWING_VOLUME_SPIKE'], 1.05),
      ...
      minBuyRatioRecentPct: parseFloatEnv(['SWING_MIN_BUY_RATIO_RECENT_PCT', 'STRATEGY_SWING_MIN_BUY_RATIO_PCT_10M'], 48),
      minNetBuyFlowUsd: parseFloatEnv(['SWING_MIN_NET_BUY_FLOW_USD', 'STRATEGY_SWING_MIN_NET_BUY_FLOW', 'MIN_NET_BUY_FLOW_USD'], 1500),
      aiConfidenceFloor: parseFloatEnv(['SWING_AI_CONFIDENCE_FLOOR', 'STRATEGY_SWING_AI_CONFIDENCE', 'AI_CONFIDENCE_FLOOR'], 70),
    ...
    solana_bull_flag_v2: {
      ...
      maxSlippagePct: parseFloat(process.env.SOLANA_BULL_FLAG_V2_MAX_SLIPPAGE_PCT || '1.5'),
      maxPriceImpactPct: parseFloat(process.env.SOLANA_BULL_FLAG_V2_MAX_PRICE_IMPACT_PCT || '1.8'),
      timeframes: String(process.env.SOLANA_BULL_FLAG_V2_TIMEFRAMES || '15m,5m').toLowerCase().split(',').map((s) => s.trim()).filter(Boolean),
    ...
    spot_day_bull_flag: {
      ...
      maxFlagUpwardDriftPct: parseFloat(process.env.BULL_FLAG_MAX_FLAG_UPWARD_DRIFT_PCT || '1'),
      ...
      latestVolumeLookbackCandles: parseInt(process.env.BULL_FLAG_VOLUME_LOOKBACK_CANDLES || '20', 10),
      latestVolumeMinRatio: parseFloat(process.env.BULL_FLAG_LATEST_VOLUME_MIN_RATIO || '2.0'),
      minSixtyMinuteMovePct: parseFloat(process.env.BULL_FLAG_MIN_60M_MOVE_PCT || '5'),
      maxSixtyMinuteMovePct: parseFloat(process.env.BULL_FLAG_MAX_60M_MOVE_PCT || '12'),
      minTargetRemainingPct: parseFloat(process.env.BULL_FLAG_MIN_TARGET_REMAINING_PCT || '20'),
      timeframes: String(process.env.BULL_FLAG_TIMEFRAMES || '15m,5m').toLowerCase().split(',').map((s) => s.trim()).filter(Boolean),
      ...
      minRR: parseFloat(process.env.BULL_FLAG_MIN_RR || '2.0'),
      expectedFeesBps: parseFloat(process.env.BULL_FLAG_EXPECTED_FEES_BPS || '30'),
