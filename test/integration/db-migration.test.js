...
test('every CREATE TABLE migration uses IF NOT EXISTS guard', () => {
  ...
  for (const f of migrations) {
    ...
    const guardedByCatalog = /IF NOT EXISTS\s*\(\s*SELECT[\s\S]+?sys\.tables/i.test(body);
    const guardedByObjectId = /IF\s+OBJECT_ID\s*\([\s\S]+?,\s*'U'\s*\)\s+IS\s+NULL/i.test(body);
    assert.ok(
      guardedByCatalog || guardedByObjectId,
...
test('migration runner supplies SQL index session options and legacy signals prerequisite', async () => {
  assert.match(_testInternals.REQUIRED_SESSION_OPTIONS, /SET QUOTED_IDENTIFIER ON/i);
  const batches = [];
  const pool = {
    request() {
      return {
        async query() { return { recordset: [{ cnt: 0 }] }; },
        async batch(sqlText) { batches.push(sqlText); },
      };
    },
  };
  await _testInternals.ensureMigrationPrerequisites(pool, { info() {} });
  assert.match(batches[0], /CREATE TABLE dbo\.signals/i);
  assert.match(batches[0], /SET QUOTED_IDENTIFIER ON/i);
});

test('dry-run branches before opening SQL or creating prerequisites', () => {
