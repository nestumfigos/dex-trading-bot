...
function createPipeline(overrides = {}) {
  ...
  const pipeline = createTokenDecisionPipeline({
    ...
    risk: {},
    strategy: {},
    AITradeBrain: overrides.AITradeBrain || { evaluateToken: async () => overrides.aiDecision || null },
...
test('AI disabled by config bypasses circuit and keeps technical signal', async () => {
  const circuit = { failures: 0, cooldownUntil: 0 };
  const { pipeline } = createPipeline({
    aiCircuit: circuit,
    aiDecision: null,
    // AITradeBrain returns null AND reports no provider enabled
    AITradeBrain: {
      evaluateToken: async () => null,
      hasAnyEnabledProvider: () => false,
    },
  });
  // Override AITradeBrain in factory: we need to re-create with the new tradeBrain
  // (the createPipeline helper hardcodes it, so we monkey-patch via execution.aiNonBlocking=false to take sync path)
  const result = await pipeline.applyAiReviewToEvaluation({
    chainName: 'kucoin',
    tokenData: { symbol: 'ABC', volume24h: 500000 },
    strategyName: 'momentum',
  ...
  // With no provider enabled, failures must NOT increment.
  assert.equal(circuit.failures, 0, 'circuit failures must not increment when no provider enabled');
});
...
test('approved BUY increments passed only after execution is confirmed', async () => {
  ...
  const cycleStats = { qualified: 0, passed: 0, executionBlocked: 0, riskBlocked: 0, gateRejectCounts: {}, rejectReasons: { other: 0 } };
  const result = await pipeline.handleApprovedTradeDecision({
    chainName: 'kucoin', exchange: {}, tokenData: { symbol: 'ABC', address: 'ABC-USDT' },
    strategyName: 'momentum', evaluation: buyEvaluation(), signalSource: 'technical', cycleStats,
    proposalDecisionId: 'proposal-id', proposal: {}, riskReview: {}, riskCheck: { allowed: true },
  });
...
test('approved BUY records execution block when no fill is confirmed', async () => {
  const { pipeline } = createPipeline({ executeBuy: async () => ({ executed: false, reason: 'execution_not_confirmed' }) });
  const cycleStats = { qualified: 0, passed: 0, executionBlocked: 0, riskBlocked: 0, gateRejectCounts: {}, rejectReasons: { other: 0 } };
  const result = await pipeline.handleApprovedTradeDecision({
    chainName: 'kucoin', exchange: {}, tokenData: { symbol: 'ABC', address: 'ABC-USDT' },
    strategyName: 'momentum', evaluation: buyEvaluation(), signalSource: 'technical', cycleStats,
