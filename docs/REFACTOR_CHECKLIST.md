...
**Status (2026-05-23):** COMPLETE for 7 of 8 sub-modules. `core.js` (final assembly that eliminates `AgentMemory` class) deferred — class proxies are stable + tested; eliminating class would require migrating 50+ call sites for negligible benefit.
...
- [X] HTML panels for `/api/health-canary` with sparklines — 2026-05-23, new `/api/health-canary/sparklines` endpoint (last N statuses per check_name) + per-row colored-bar SVG sparkline (green/amber/red = PASS/WARN/FAIL). 3-sigma badges still deferred as cosmetic.
...
**Status (2026-05-23):** 16.5.1 SHIPPED. Remaining 3 items DEFERRED with rationale below.
- [X] `ai_prompts` table → live loader — 2026-05-23, `src/ai/prompt-loader.js` (sync cache + bg prefetch + fallback), wired into `buildEnsemblePrompt`. Seed script `scripts/seed-ai-prompts.js`. Tests pass (7/7). When `dbo.ai_prompts` has active row, template overrides inline; else inline fallback. Run `node scripts/seed-ai-prompts.js` to populate.
