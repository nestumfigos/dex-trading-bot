 ...
 *
 * Owns the body of `scanChain(chainName, exchange, strategyName, options)`:
 *  - exchange availability gate
 *  - status updates + UI sync
 *  - KuCoin ticker refresh, new-listing prepend, catalyst priority
 *  - token discovery + BSC ranking + rotating window
 *  - batched processToken loop with throttling
 *  - error envelope + recordExchangeSuccess/Failure
 *
 * Behavior IDENTICAL to prior inline implementation; this module exists for
 * testability + readability. All collaborators are dep-injected so this file
 * has zero direct module-level imports of bot state.
 */
...
function createMomentumScanner(deps = {}) {
  const {
    ...
    processToken,
    sleep,
    CHAIN_LABELS, // not used in body, accepted for symmetry
  } = deps;
