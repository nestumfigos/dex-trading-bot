...
const telegramConfigured = Boolean(config.telegram.token && config.telegram.chatId);
const telegramApiBase = telegramConfigured
...
async function sendHeartbeat(stats = null) {
  ...
  if (unhealthyReasons.length > 0) {
    healthNote = `\n🔴 Unhealthy: ${unhealthyReasons.slice(0, 3).join(', ')}`;
  } else if (degradedReasons.length > 0) {
    healthNote = `\n🟡 Degraded: ${degradedReasons.slice(0, 2).join(', ')}`;
  }
...
async function sendHealthAlert(healthStatus) {
  ...
  if (unhealthyReasons.length > 0) {
    const reasons = unhealthyReasons.join('\n  • ');
    await sendAlert(`🔴 <b>Bot UNHEALTHY</b>\n  • ${reasons}`);
  } else if (degradedReasons.length > 0) {
    const reasons = degradedReasons.join('\n  • ');
