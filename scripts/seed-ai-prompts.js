 ...
 *
 * Idempotent — UPSERTs by (name, scope). Marks new row active=1 and bumps
 * version. The bot's prompt-loader picks up the new template within
 * AI_PROMPT_CACHE_TTL_MS (default 5min).
 *
 * Run with SQL_ENABLED=true:
 *   node scripts/seed-ai-prompts.js                  → scope=global
 *   node scripts/seed-ai-prompts.js --scope=live     → scope=live
 *   node scripts/seed-ai-prompts.js --dry-run        → print SQL, don't write
 */
...
const PROMPTS = [
  {
    ...
    provider: null, // shared across all providers
