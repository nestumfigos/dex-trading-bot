...
async function main() {
  ...
  if (datasetHash && datasetHash !== expectedHash) {
    throw new Error(`Feature schema mismatch for ${framework}: dataset=${datasetHash} runtime=${expectedHash}`);
  }
  const artifactMap = {
  ...
  logger.info(`[ProductionML] trained ${framework} artifact at ${result.artifactPath} using ${result.trainedRows} rows | schema=${expectedHash.slice(0, 12)}`);
}
