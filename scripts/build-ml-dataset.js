...
async function main() {
  ...
  for (const entries of grouped.values()) {
    ...
    for (let i = 0; i < entries.length - horizon; i += 1) {
      ...
      const parity = summarizeFeatureParity(current.features, FEATURE_ORDER);
      dataset.push({
        ...
        featureCoverage: parity.coverage,
      });
    }
  }
  ...
  fs.writeFileSync(outPath, JSON.stringify({
    ...
    featureSchemaHash: getFeatureSchemaHash(FEATURE_ORDER, FEATURE_SCHEMA_VERSION),
    rows: dataset,
    metadata: {
